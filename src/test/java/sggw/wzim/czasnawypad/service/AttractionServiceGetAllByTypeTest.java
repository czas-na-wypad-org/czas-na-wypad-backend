...
@DisplayName("Attraction Service Get ALl By Type Test")
class AttractionServiceGetAllByTypeTest {
    ...
    @ParameterizedTest
    @MethodSource
    void getAllAttractionsByType(List<Attraction> attractions, List<AttractionDTO> expected) {
        // given
        String type = "monument";
        when(attractionRepository.findAllByIsDeletedFalseAndType(type)).thenReturn(attractions);
        when(attractionDTOMapper.fromList(attractions)).thenReturn(expected);

        // when
        List<AttractionDTO> actual = underTest.getAllAttractionsByType(type);

        // then
        assertEquals(expected, actual);
    }

    private static Stream<Arguments> getAllAttractionsByType() {
        return Stream.of(
                Arguments.of(
                        List.of(), List.of()
                ),
    ...
    private static AttractionDTO getAttractionDTO(String name, String type) {
    ...
    private static Attraction getAttraction(Integer id, String name, String type) {
